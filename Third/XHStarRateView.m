...
-(instancetype)initWithNumberOfStars:(NSInteger)numberOfStars rateStyle:(RateStyle)rateStyle isAnination:(BOOL)isAnimation finish:(finishBlock)finish {
    if (self = [self init]) {
        _numberOfStars = numberOfStars;
        _rateStyle = rateStyle;
        _isAnimation = isAnimation;
...
- (void)setUpShowStarScore:(CGFloat)score {
